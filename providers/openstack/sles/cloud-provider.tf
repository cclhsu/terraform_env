resource "null_resource" "generate_cloud_provider_conf" {
  depends_on = [
    null_resource.etcd_reboot,
    null_resource.storage_reboot,
  ...
  provisioner "local-exec" {
    environment = {
      ...
      TR_ETCD_IPS          = join(" ", openstack_networking_floatingip_v2.etcd_ext.*.address)
      TR_STORAGE_IPS       = join(" ", openstack_networking_floatingip_v2.storage_ext.*.address)
