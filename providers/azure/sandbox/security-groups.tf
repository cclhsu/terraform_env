locals {
  ...
  # security_rules_etcd = local.security_rules_commons
  # security_rules_storage = local.security_rules_commons
...
#     local.tags,
#     {
#       Name = "${var.stack_name}-security-group-etcd"
#     },
#   )
# }

# resource "azurerm_network_security_rule" "etcd" {
#   count                       = length(local.security_rules_etcd)
#   resource_group_name         = azurerm_resource_group.resource_group.name
#   network_security_group_name = azurerm_network_security_group.etcd.name
#   name                        = element(local.security_rules_etcd, count.index)["name"]
#   priority                    = (100 + count.index)
#   direction                   = element(local.security_rules_etcd, count.index)["direction"]
#   access                      = element(local.security_rules_etcd, count.index)["access"]
#   protocol                    = element(local.security_rules_etcd, count.index)["protocol"]
#   source_port_range           = element(local.security_rules_etcd, count.index)["source_port_range"]
#   destination_port_range      = element(local.security_rules_etcd, count.index)["destination_port_range"]
#   source_address_prefix       = element(local.security_rules_etcd, count.index)["source_address_prefix"]
#   destination_address_prefix  = element(local.security_rules_etcd, count.index)["destination_address_prefix"]
# }
...
#     local.tags,
#     {
#       Name = "${var.stack_name}-security-group-storage"
#     },
#   )
# }

# resource "azurerm_network_security_rule" "storage" {
#   count                       = length(local.security_rules_storage)
#   resource_group_name         = azurerm_resource_group.resource_group.name
#   network_security_group_name = azurerm_network_security_group.storage.name
#   name                        = element(local.security_rules_storage, count.index)["name"]
#   priority                    = (100 + count.index)
#   direction                   = element(local.security_rules_storage, count.index)["direction"]
#   access                      = element(local.security_rules_storage, count.index)["access"]
#   protocol                    = element(local.security_rules_storage, count.index)["protocol"]
#   source_port_range           = element(local.security_rules_storage, count.index)["source_port_range"]
#   destination_port_range      = element(local.security_rules_storage, count.index)["destination_port_range"]
#   source_address_prefix       = element(local.security_rules_storage, count.index)["source_address_prefix"]
#   destination_address_prefix  = element(local.security_rules_storage, count.index)["destination_address_prefix"]
# }
