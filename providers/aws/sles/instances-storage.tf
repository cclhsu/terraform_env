...
# data "template_file" "storage_register_suma" {
...
#     register_suma   = var.suma_server_name == "" ? join("\n", data.template_file.storage_register_suma.*.rendered) : ""
...
# data "template_cloudinit_config" "storage_cfg" {
#   gzip          = false
#   base64_encode = true

#   part {
#     content_type = "text/cloud-config"
#     content      = data.template_file.storage_cloud-init.rendered
#   }
# }

resource "aws_iam_instance_profile" "storage" {
  name  = "${var.stack_name}-storage"
  role  = aws_iam_role.storage[count.index].name
  count = length(var.iam_profile_storage) == 0 ? 1 : 0
}

resource "aws_iam_role" "storage" {
  name        = "${var.stack_name}-storage"
  description = "IAM role needed by CPI on storage nodes"
  path        = "/"
  count       = length(var.iam_profile_storage) == 0 ? 1 : 0
...
resource "aws_iam_role_policy" "storage" {
  name  = "${var.stack_name}-storage"
  role  = aws_iam_role.storage[count.index].id
  count = length(var.iam_profile_storage) == 0 ? 1 : 0
...
# it's applied only to storage nodes.
#
...
# alter to add the rules needed to access the storage nodes from the AWS
# resources dynamically provisioned by the CPI (eg: load balancers).
resource "aws_security_group" "storage" {
  description = "security group rules for storage node"
  name        = "${var.stack_name}-storage"
...
resource "aws_instance" "storage" {
  ...
  iam_instance_profile        = length(var.iam_profile_storage) == 0 ? aws_iam_instance_profile.storage.0.name : var.iam_profile_storage
  # ebs_optimized          = true
