...
#     register_suma   = var.suma_server_name == "" ? join("\n", data.template_file.etcd_register_suma.*.rendered) : ""
...
#     content      = data.template_file.etcd_cloud-init.rendered
#   }
# }
...
# it's applied only to etcd nodes.
#
...
# alter to add the rules needed to access the etcd nodes from the AWS
# resources dynamically provisioned by the CPI (eg: load balancers).
resource "aws_security_group" "etcd" {
  description = "security group rules for etcd node"
  name        = "${var.stack_name}-etcd"
...
resource "aws_instance" "etcd" {
  ...
  iam_instance_profile        = length(var.iam_profile_etcd) == 0 ? aws_iam_instance_profile.etcd.0.name : var.iam_profile_etcd
  # ebs_optimized          = true
