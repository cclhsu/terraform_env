...
data "template_file" "storage_cloud-init" {
  ...
  vars = {
    ...
    register_suma   = var.suma_server_name == "" ? join("\n", data.template_file.storage_register_sumba.*.rendered) : ""
...
data "template_cloudinit_config" "storage_cfg" {
  ...
  part {
    ...
    content      = data.template_file.storage_cloud-init[count.index].rendered
  }
}
...
resource "aws_instance" "storage" {
  ...
  user_data                   = data.template_cloudinit_config.storage_cfg.rendered
  iam_instance_profile        = length(var.iam_profile_storage) == 0 ? aws_iam_instance_profile.storage.0.name : var.iam_profile_worker
  # ebs_optimized          = true
